...
#  좋은 프로그래머의 특징
기본에 충실함
    레퍼런스(스펙, 매뉴얼 등)를 읽고 이해하려 노력한다.
    사용하는 툴의 기능과 한계를 잘 이해한다.
    오류 메시지를 주의 깊게 읽고 해석한다.
문제 해결력
    문제를 최대한 단순화해서 이해하고, 그 기반 위에서 해결책을 찾는다.
    추측하지 않고, 사실과 근거를 기반으로 문제를 분석한다.
코드와 도메인 이해
    코드 읽기/쓰기를 반복하며 기술적 지식과 비즈니스 도메인에 대한 이해를 높인다.
    코드를 단순화하고 구조화하는 능력을 기른다.
협업과 나눔
    다른 사람을 도우려는 태도를 가진다.
    글쓰기를 통해 지식을 공유하고, 생각을 정리하는 능력을 키운다.
태도와 마인드셋
    지위보다 명성을 추구한다.
    인내심, 집중력, 헌신을 기반으로 성장한다.
    환경 탓을 하지 않고, 모른다는 것을 인정하며 배움의 자세를 갖는다.
    지름길은 없다는 사실을 받아들이고 꾸준히 노력한다.
