...
try:
    login(token=HF_TOKEN)  # 여기에 당신의 Hugging Face API 토큰을 입력하세요
except ImportError:
...
# 토크나이저 및 모델 로드 (인증 토큰 사용)
tokenizer = AutoTokenizer.from_pretrained(model_name, use_auth_token=True)
tokenizer.pad_token = tokenizer.eos_token
model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=model_path, use_auth_token=True).to(device)
